...
function buildMcpHandler(
  ...
  return createMcpHandler(
    (context) => {
      const clientIp = context.requestInfo
        ? getClientIp(context.requestInfo)
        : undefined;
      return createServer({
        provider,
        logger,
    ...
    },
    {
      ...
      responseMode: "json",
      onerror: (error) =>
        logger.error("mcp_handler_error", { errorType: error.name }),
    },
  );
}
...
export function createHttpServer({
  ...
  const nodeHandler = toNodeHandler(mcpHandler, {
    onerror: (error) =>
      logger.error("http_adapter_error", { errorType: error.name }),
  });
    ...
    try {
      ...
      if (error instanceof RequestBodyError) {
        sendJson(response, error.status, {
          jsonrpc: "2.0",
          error: {
            code: error.status === 400 ? -32700 : -32600,
...
async function main(): Promise<void> {
  ...
  const { server, closeMcpHandler } = createHttpServer({
    config,
    logger,
    provider,
  });
